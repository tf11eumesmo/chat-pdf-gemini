...
# CSS Personalizado (Incluindo o Cabeçalho Fixo e Fonte Menor)
st.markdown("""
<style>
    /* --- Estilos Gerais --- */
    .correct-answer {
    ...
    .sticky-header {
        position: sticky;
        top: 0;
        background-color: white; /* Cor de fundo para cobrir o conteúdo ao rolar */
        z-index: 999;
        padding-bottom: 10px;
        margin-bottom: 15px;
    ...
    /* Título do Chat (Fonte Menor) */
    .chat-header-small {
        font-size: 1.1rem !important; /* Fonte reduzida */
        font-weight: 600;
        margin-top: 10px;
        margin-bottom: 5px;
        color: #555;
...
# --- FUNÇÃO DE FORMATAÇÃO (Definida antes do uso) ---
def formatar_resposta(texto):
    """Formata a resposta para diferentes tipos de questão"""
    if not texto:
        return ""
    
    # Remover tags HTML indesejadas iniciais
...
# --- ÁREA PRINCIPAL (COM CABEÇALHO FIXO) ---

# Container Sticky para o cabeçalho
...
if st.session_state.pdf_content:
    ...
    <div class="materia-info" style="background-color: #fff3cd; border-left-color: #ffc107; color: #856404;">
        <strong>⚠️ Nenhuma matéria selecionada</strong>
    </div>
    """, unsafe_allow_html=True)

st.divider()

# 3. Título do Chat (Fonte Menor)
st.markdown('<h3 class="chat-header-small">💬 Chat de Dúvidas</h3>', unsafe_allow_html=True)

# Fecha a div sticky
st.markdown('</div>', unsafe_allow_html=True)

# --- LOOP DE MENSAGENS (ÁREA DE ROLAGEM) ---
